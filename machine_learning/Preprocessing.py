...
class Preprocessing:
    ...
    def __remove_extreme_areas(self):
        ...
        for attr in ["sale_train", "rent_train", "sale_test", "rent_test"]:
            df = getattr(self, attr)
            df = df[(df["area_m2"] >= 20) & (df["area_m2"] <= 500)]
            setattr(self, attr, df)
    ...
    def __remove_duplicates(self):
        for attr in ["sale_train", "rent_train", "sale_test", "rent_test"]:
            df = getattr(self, attr)
            df.drop_duplicates(subset=["area_m2", "district_name", "floor", "street_address"])
            setattr(self, attr, df)
    ...
    def run(self):
        ...
        print("Rows Removed (Sale): ", before_sum_sale - after_sale_sum)
        print("Rows Removed (Rent): ", before_sum_rent - after_rent_sum)
        print("After Preprocessing (Sale): ", after_sale_sum)
        print("After Preprocessing (Rent): ", after_rent_sum)
